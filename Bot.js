class Bot{
  ...
  async Think(CurrentField, Device){
    ...
    for (let i = 0; i < 5; ++i){
      for (let j = 0; j < 5; ++j){
        if (CurrentField.Data[i][j].Type == "Empty"){
          ...
          let Score = await this.Estimate(CurrentField, 0, "Human", -Infinity, Infinity, Device);
  ...
  async Estimate(GameField, Depth, Player, Alpha, Beta, Device){
